...
public class RelatorioControllerAvaliacao extends HttpServlet {
...
private void relatorioSemParametro(HttpServletRequest request, HttpServletResponse response) {
 Connection conexao = null;
        try {
            ...
            String relatorio = getServletContext().getRealPath("/WEB-INF/classes/relatorio")+"/reportAvaliacaoSemParamentro.jasper";
...
private void relatorioComParametro(HttpServletRequest request, HttpServletResponse response) {
 Connection conexao = null;
        try {
            ...
            HashMap parametros = new HashMap();
            parametros.put("P_ALUNO", Integer.parseInt(request.getParameter("txtMatricula")));
            String relatorio = getServletContext().getRealPath("/WEB-INF/classes/relatorio")+"/reportAvalicao.jasper";
            JasperPrint jp = JasperFillManager.fillReport(relatorio, parametros, conexao);
            byte[] relat = JasperExportManager.exportReportToPdf(jp);
            response.setHeader("Content-Disposition", "attachment;filename=relatorio.pdf");
            response.setContentType("application/pdf");
